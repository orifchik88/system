...
//        DB::beginTransaction();
        try {
            $dxa = new DxaResponse();
            $dxa->task_id = $this->argument('task_id');
            $dxa->task = $response->body();
            $dxa->dxa_response_statuses_id = 1;
            $dxa->email = $data['legal_entity_email']['real_value'];
            ...
            $dxa->phone = $data['phone']['real_value'];
...
//            $dxa->technic_org_name = $data['name_org']['real_value'];
            ...
            foreach ($data['info_supervisory']['value'] as $key => $item) {
                ...
                $dxaResSupervisor->dxa_response_id = $dxa->id;
